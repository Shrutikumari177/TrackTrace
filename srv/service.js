...
module.exports = async (srv) => {
    ...
    srv.on('getBatchIDRelevantData', async (req) => {
        try {
            const { BatchNo, filterNonEmptyBoxQRCode } = req.data;
            ...
            // Apply filter for non-empty BoxQRCode and sort by ICID 
        ...
        try {
            const existingBoxes = await cds.run(
                SELECT.from('track.MaterialBox')
                    .columns(['SerialNo', 'IC_ICID']) // Check SerialNo and linked IC_ICID
